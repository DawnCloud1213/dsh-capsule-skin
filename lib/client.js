...
body[data-ds-skin="capsule"] {
  ...
  /* 胶囊模糊半径变量（设置 → 壁纸皮肤 → 美化编辑器「滤镜」区可调，全局等比联动） */
  ...
  /* 圆角体系（dsh 官方无 radius token，皮肤自建，语义化命名） */
  --dsh-radius-xl: 24px;        /* 左栏大胶囊 */
  --dsh-radius-rail: 16px;      /* 收起窄栏 */
  --dsh-radius-lg: 18px;        /* 顶栏胶囊 / 项目组胶囊 */
  --dsh-radius-md: 14px;        /* 侧栏按钮/条目/搜索 / 输入内层 / 代码块 */
  --dsh-radius-sm: 12px;        /* 项目行 */
  --dsh-radius-xs: 10px;        /* 会话行 / 设置页卡片 */
  --dsh-radius-composer: 22px;  /* 输入托盘 */
  --dsh-radius-pill: 999px;     /* 药丸（行内代码） */
  /* 玻璃透明度体系（color-mix 的 alpha，基于官方 token 派生） */
  --dsh-glass-capsule: 55%;     /* 左栏/顶栏 主表面 */
  --dsh-glass-item: 62%;        /* 侧栏按钮/条目 / 输入内层 / 行内代码 */
  --dsh-glass-group: 45%;       /* 项目组胶囊 / 输入托盘外层 */
  --dsh-glass-bubble: 68%;      /* 对话气泡 */
  --dsh-glass-code: 78%;        /* 代码块 */
  --dsh-glass-add: 60%;         /* 加号/历史小按钮 */
  --dsh-glass-session: 8%;      /* 会话行（品牌色微染） */
  --dsh-glass-banner: 55%;      /* 代码块 banner */
}
...
body[data-ds-skin="capsule"] [class*="sidebarCol"] {
  ...
  border-radius: var(--dsh-radius-xl) !important;
...
body[data-ds-skin="capsule"] [class*="sidebarCol"]::before {
  ...
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(46, 48, 58)) var(--dsh-glass-capsule), transparent);
...
body[data-ds-skin="capsule"] [class*="frame"][data-sidebar-collapsed] [class*="sidebarCol"] {
  margin: 0 !important;
  border-radius: var(--dsh-radius-rail) !important;
}
/* ── 顶栏（会话标题栏）：胶囊悬浮条，与左栏呼应；滚动分隔细线隐藏 ── */
body[data-ds-skin="capsule"] [class*="centerCol"] header[class*="header"] {
  margin: 12px 16px 0 !important;
  border-radius: var(--dsh-radius-lg) !important;
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(46, 48, 58)) var(--dsh-glass-capsule), transparent) !important;
  border: 1px solid color-mix(in srgb, var(--dsw-alias-border-l1, rgb(192, 194, 210)) 12%, transparent) !important;
  backdrop-filter: blur(var(--dsh-capsule-blur-md)) saturate(140%);
  -webkit-backdrop-filter: blur(var(--dsh-capsule-blur-md)) saturate(140%);
  box-shadow:
...
body[data-ds-skin="capsule"] [class*="sidebarCol"] [class*="search"] {
  border-radius: var(--dsh-radius-md) !important;
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(46, 48, 58)) var(--dsh-glass-item), transparent) !important;
...
body[data-ds-skin="capsule"] [class*="sidebarCol"] [class*="groupSection"]:has([class*="projectRow"]) {
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(46, 48, 58)) var(--dsh-glass-group), transparent) !important;
  border: 1px solid color-mix(in srgb, var(--dsw-alias-border-l1, rgb(192, 194, 210)) 12%, transparent) !important;
  border-radius: var(--dsh-radius-lg) !important;
...
body[data-ds-skin="capsule"] [class*="sidebarCol"] [role="treeitem"][class*="projectRow"] {
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-1, rgb(29, 31, 41)) var(--dsh-glass-capsule), transparent) !important;
  border-radius: var(--dsh-radius-sm) !important;
...
body[data-ds-skin="capsule"] [class*="sidebarCol"] [role="treeitem"][class*="sessionRow"] {
  background: color-mix(in srgb, var(--dsw-alias-brand-primary, rgb(238, 239, 255)) var(--dsh-glass-session), transparent) !important;
  border: 1px solid color-mix(in srgb, var(--dsw-alias-brand-primary, rgb(238, 239, 255)) 6%, transparent) !important;
  border-radius: var(--dsh-radius-xs) !important;
...
body[data-ds-skin="capsule"] [class*="centerCol"] [class*="bubble"] {
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(46, 48, 58)) var(--dsh-glass-bubble), transparent) !important;
...
body[data-ds-skin="capsule"] [class*="centerCol"] [class*="_markdown_"] :not(pre) > code {
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(40, 42, 52)) var(--dsh-glass-item), transparent) !important;
  ...
  border-radius: var(--dsh-radius-pill) !important;
...
body[data-ds-skin="capsule"] [class*="centerCol"] [class*="md-code-block"] {
  --dsl-code-block-border-radius: var(--dsh-radius-md);
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-1, rgb(24, 26, 34)) var(--dsh-glass-code), transparent) !important;
...
body[data-ds-skin="capsule"] [class*="centerCol"] [class*="md-code-block"] [class*="banner"] {
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(40, 42, 52)) var(--dsh-glass-banner), transparent) !important;
}
/* 输入区「命令」加号按钮：浅色底 + 白色图标 → 主题色胶囊 */
body[data-ds-skin="capsule"] [class*="composerSeat"] button[class$="_add"] {
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(40, 42, 52)) var(--dsh-glass-add), transparent) !important;
...
body[data-ds-skin="capsule"] [class*="centerCol"] [class*="card"]:has(textarea) {
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-1, rgb(29, 31, 41)) var(--dsh-glass-group), transparent) !important;
  border: 1px solid color-mix(in srgb, var(--dsw-alias-border-l1, rgb(192, 194, 210)) 14%, transparent) !important;
  border-radius: var(--dsh-radius-composer) !important;
...
[data-capsule-skin-section] [data-capsule-skin-option] {
  ...
  gap: 6px;
  padding: 8px;
  border-radius: var(--dsh-radius-xs);
  cursor: pointer;
...
[data-capsule-skin-section] [data-skin-thumb-bubble-ai] {
  ...
  background: color-mix(in srgb, var(--dsw-alias-bg-layer-2, rgb(46, 48, 58)) var(--dsh-glass-bubble), transparent);
...
[data-capsule-skin-section] [data-capsule-skin-rowbtn] {
  ...
  margin-top: 8px; padding: 7px 10px; border-radius: var(--dsh-radius-xs); cursor: pointer;
...
/* ── 原生控件统一暗色（标准接口：color-scheme 继承属性）──────────
   覆盖逻辑分三层：
   1) 官方组件（含自研 Menu/下拉/窗口）：全部走 --dsw-* token（定义在 body，
      body[data-ds-dark-theme] 自动切换明暗），overrideTokens 已覆盖 → 无需逐组件修补。
   2) 系统渲染控件（select/option/checkbox/radio/date/滚动条等）：不吃 token，
      唯一标准接口是 CSS color-scheme（继承属性）。dsh 只在 <html> 设过一次，
      对带自定义样式的 select/option 覆盖不足。
   3) option 显式着色兜底：原生下拉弹层在浅色系统配色下是白底，而 option 文字
      继承皮肤的浅色 → 白底白字；显式背景+文字色双保险（Chrome 生效）。 */
body[data-ds-skin="capsule"] :is(select, option, input, textarea, button) {
  color-scheme: dark !important;
}
body[data-ds-skin="capsule"] option {
  color: var(--dsw-alias-label-primary, #e8ecf4) !important;
  background-color: var(--dsw-alias-bg-layer-3, rgb(36, 38, 48)) !important;
}
body[data-ds-skin="capsule"] option:hover,
body[data-ds-skin="capsule"] option:checked {
  background-color: color-mix(in srgb, var(--dsw-alias-brand-primary, rgb(238, 239, 255)) 22%, var(--dsw-alias-bg-layer-3, rgb(36, 38, 48))) !important;
}
/* 自研下拉/菜单组件（官方 Menu 已走 token 无需处理；第三方插件若用
   role 化列表却硬编码浅色，至少让 color-scheme 保持暗色，避免混合渲染） */
body[data-ds-skin="capsule"] :is([role="listbox"], [role="option"], [role="combobox"], [role="menuitem"], [role="menu"]) {
  color-scheme: dark !important;
}
`;
...
// 胶囊模糊行（美化面板内，紧跟「模糊（壁纸）」之后）：全局 UI 毛玻璃，独立于 LAB 壁纸参数。
// 数据铁律：不注册 ui.sliderRefs、不写 settings、不触发 scheduleSave——
// 否则会被 refreshControls / 预设 /「重置」污染；预设与重置只作用于壁纸效果。
function capsuleBlurRow() {
  ...
  note.textContent = '胶囊毛玻璃全局联动（左栏/顶栏/输入托盘）· 0px = 无模糊 · 默认 16px';
...
function createWallpaperLab(ctx, getBgColor) {
  ...
  const state = { bgUrl: null, videoUrl: null, fallbackTried: false, disposed: false, bgSize: null };
  ...
  // 动态背景层：Wallpaper Engine 视频壁纸专用 <video>（循环静音自动播放）
  // 层叠于 bgLayer（海报）之上、美化滤镜层（shade/vignette/tint/grain）之下；
  // 取景/缩放 transform 与 bgLayer 镜像同步（见 applyBgTransform）。
  const videoLayer = document.createElement('div');
  videoLayer.setAttribute('data-wpl-video', '');
  videoLayer.style.cssText = 'position: fixed; inset: 0; z-index: -1; pointer-events: none; overflow: hidden; display: none;';
  const videoEl = document.createElement('video');
  videoEl.style.cssText = 'position: absolute; left: 0; top: 0; width: 100%; height: 100%; object-fit: cover;';
  videoEl.muted = true;
  videoEl.loop = true;
  ...
  function applyBgTransform() {
    ...
    // 视频层镜像同一 transform：背景图在元素内以左上角为 anchor 画 iw×ih，
    // 与 video 元素 left:0/top:0 + width/height=iw×ih 的坐标系完全一致
    if (state.videoUrl && videoEl) {
      videoEl.style.objectFit = 'fill';
      videoEl.style.width = iw + 'px';
      videoEl.style.height = ih + 'px';
      videoEl.style.transformOrigin = '0 0';
      videoEl.style.transform = 'translate(' + tx + 'px, ' + ty + 'px) scale(' + z + ')';
    }
  }

  // 视频层回退基础态：cover 铺满视口（美化关闭 / 尺寸未就绪时保证仍动态显示）
  function resetVideoBase() {
    if (!videoEl || !state.videoUrl) return;
  ...
  function applyAll() {
    ...
    if (!on) {
      resetVideoBase(); // 美化关闭：视频层保持 cover 基础态，动态照常播放
    ...
    if (state.videoUrl && videoEl) videoEl.style.filter = bgLayer.style.filter;
    applyBgTransform();
  ...
  function renderPanelInto(container, onClose) {
    ...
    for (const key of FILTER_KEYS) {
      ...
      // 胶囊模糊（全局毛玻璃）紧跟壁纸模糊之后
      if (key === 'blur') container.appendChild(capsuleBlurRow());
    }
  ...
  // ---------- 动态背景：Wallpaper Engine 视频壁纸 ----------
  // videoUrl 由 applyNow 传入切换；海报/尺寸仍走 setBg（body 背景 = 海报，自适应取色正常）。
  // 视频真实尺寸在 loadedmetadata 后回填 state.bgSize → cover/取景变换对视频生效。
  function setVideo(videoUrl, posterUrl) {
    if (state.disposed) return;
    if (!videoUrl) {
      state.videoUrl = null;
      if (videoEl) {
    ...
    if (videoUrl === state.videoUrl) { videoLayer.style.display = ''; return; }
  ...
  // 视频元数据 → 尺寸：cover 构图 / 取景 transform 依赖与图片共用 state.bgSize
  videoEl.addEventListener('loadedmetadata', () => {
    if (state.disposed || !state.videoUrl) return;
  ...
  function mountLayers() {
    ...
    document.body.append(bgLayer, videoLayer, shadeLayer, vignetteLayer, tintLayer, grainLayer, frameLayer, textLayer);
  ...
  function dispose() {
    ...
    if (videoEl) { videoEl.removeAttribute('src'); try { videoEl.load(); } catch (e) {} }
    videoLayer.remove();
    textLayer.remove();
    frameLayer.remove();
    grainLayer.remove();
...
module.exports = {
  ...
  apply(ctx) {
    ...
    let weDirOk = false; // Wallpaper Engine 目录检测状态（列表分组提示用）

    let lastApplyId = null;
    const applyNow = (skin) => {
      disposeTokens = applySkinTokens(ctx, skin, disposeTokens);
      // 背景：video 类 → body 底 = 海报（自适应取色/回退用），美化层挂 <video>（真动态）
      // 其他（image/gif/poster）→ 直接作 body 背景（gif 背景 CSS 自带动画）
      if (skin.kind === 'video') {
        body.style.backgroundImage = `url("${skin.poster || skin.bg}")`;
        lab.setVideo(skin.video, skin.poster || skin.bg);
      } else {
        lab.setVideo(null);
    ...
    const applySkin = (skin) => {
      // WE 壁纸 tokens 懒加载（首次 /we-wallpaper/color 跑 python 取色，服务端 mtime 缓存；
      // 期间不阻塞，取回后经 lastApplyId 去抖只应用最后选中的那张）
      if (skin && skin.origin === 'we' && !skin.tokens) {
        lastApplyId = skin.id;
        fetch('/we-wallpaper/color?id=' + encodeURIComponent(skin.weId), { cache: 'no-store' })
          .then(r => r.json())
          .then(d => {
            if (skin.tokens === undefined || skin.tokens === null) skin.tokens = d && d.ok ? d.tokens : {};
            if (lastApplyId === skin.id) applyNow(skin);
          })
          .catch(() => {
            if (skin.tokens === undefined || skin.tokens === null) skin.tokens = {};
            if (lastApplyId === skin.id) applyNow(skin);
          });
        return;
      }
    ...
    // ── 设置 → 壁纸皮肤：列表视图（壁纸网格 + 美化入口 + 目录/扫描） ──
    // 胶囊模糊已并入美化编辑器（滤镜区「模糊」之后），此处不再有「外观」分组
    const buildListView = (container, onOpenBeautify) => {
      ...
      const localSkins = skins.filter(s => s.origin !== 'we');
      const weSkins = skins.filter(s => s.origin === 'we');

      // 单个壁纸项：缩略图（壁纸为底 + 迷你 UI 骨架）+ 名称 + WE 徽标 + 本地壁纸删除
      const makeOption = (gridEl, skin) => {
        ...
        if (skin.origin === 'we') {
          ...
          badge.setAttribute('data-capsule-skin-badge', '');
          badge.textContent = skin.kind === 'video' ? '▶ 视频' : (skin.kind === 'gif' ? '🎞 动图' : '🖼 图片');
          badge.style.cssText = 'position: absolute; top: 4px; left: 6px; z-index: 2; font-size: 10px; padding: 1px 5px; border-radius: 5px; background: color-mix(in srgb, var(--dsw-alias-brand-primary, rgb(238,239,255)) 26%, transparent); color: var(--dsw-alias-label-primary, #fff);';
        }
        // 删除（仅本地壁纸；两次点击确认，防误删；删除当前壁纸自动切到第一个）
        let del = null;
        if (skin.origin !== 'we') {
        ...
        if (del) opt.appendChild(del);
        opt.addEventListener('click', () => {
          applySkin(skin);
          // 就地高亮（不关闭设置，方便对比挑选）
          for (const o of gridEl.querySelectorAll('[data-capsule-skin-option]')) o.removeAttribute('data-active');
      ...
      for (const skin of localSkins) makeOption(grid, skin);

      // Wallpaper Engine 分组：WE 源壁纸（流式直读，不可删除）
      if (weSkins.length) {
        const secWe = document.createElement('div');
        secWe.setAttribute('data-capsule-skin-sec', '');
        secWe.textContent = '🎬 Wallpaper Engine (' + weSkins.length + ')';
        container.appendChild(secWe);
        const gridWe = document.createElement('div');
        gridWe.setAttribute('data-capsule-skin-grid', '');
        container.appendChild(gridWe);
        for (const skin of weSkins) makeOption(gridWe, skin);
        if (!weDirOk) {
          const weNote = document.createElement('div');
          weNote.setAttribute('data-capsule-skin-note', '');
          weNote.textContent = '目录可通过 DSH_SKIN_WE_DIR 环境变量或 /we-wallpaper/cfg 指定';
          container.appendChild(weNote);
        }
      }
      ...
      // Wallpaper Engine 目录（默认自动探测 SteamLibrary；可在此手动指定）
      const secWeDir = document.createElement('div');
      secWeDir.setAttribute('data-capsule-skin-sec', '');
      secWeDir.textContent = 'Wallpaper Engine 目录';
      container.appendChild(secWeDir);
      const weDirInput = document.createElement('input');
      weDirInput.setAttribute('data-capsule-skin-dir', '');
      weDirInput.value = '…';
      weDirInput.style.cssText = dirInput.style.cssText;
      const weDirStatus = document.createElement('div');
      weDirStatus.style.cssText = 'margin-top: 4px; font-size: 11px; opacity: 0.7;';
      const weDirSave = document.createElement('button');
      weDirSave.textContent = '保存 WE 目录';
      weDirSave.style.cssText = dirSave.style.cssText;
      weDirSave.addEventListener('click', async () => {
        const dir = weDirInput.value.trim();
        if (!dir) { weDirSave.textContent = '⚠️ 目录不能为空'; return; }
        weDirSave.textContent = '⏳ 保存中…';
        try {
          const r = await fetch('/we-wallpaper/cfg', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ weDir: dir }),
          });
          const data = await r.json();
          weDirSave.textContent = data.ok ? '✅ 已保存' : '⚠️ ' + (data.error || '失败');
        } catch (e) {
          weDirSave.textContent = '⚠️ 保存失败';
        }
      });
      container.appendChild(weDirInput);
      container.appendChild(weDirSave);
      container.appendChild(weDirStatus);
      fetch('/we-wallpaper/cfg', { cache: 'no-store' })
        .then(r => r.json())
        .then(cfg => {
          weDirInput.value = cfg.weDir || '';
          weDirStatus.textContent = cfg.weDirExists ? '✅ 目录可用' : (cfg.weDir ? '⚠️ 目录不存在' : '未配置（自动探测）');
        })
        .catch(() => {});
    ...
    // 加载壁纸清单：本地 skins.json + Wallpaper Engine 目录 合并（热添加：刷新页面即见）
    const loadSkins = async () => {
      let list = [];
      try {
        const r = await fetch('/skin-assets/skins.json', { cache: 'no-store' });
        list = await r.json();
      } catch (e) { list = []; }
      let weItems = [];
      try {
        const r = await fetch('/we-wallpaper/index', { cache: 'no-store' });
        const cat = await r.json();
        if (cat && cat.ok) {
          weDirOk = !!cat.weDirExists;
          weItems = Array.isArray(cat.items) ? cat.items : [];
        }
      } catch (e) { /* WE 不可用则仅本地 */ }
      skins = Array.isArray(list) ? list : [];
      const we = weItems
        .filter(i => i.kind) // application/无描述项已由 host 过滤
        .map(i => ({
          id: i.id,
          weId: i.weId,
          origin: 'we',
          kind: i.kind === 'video' ? 'video' : (i.kind === 'gif' ? 'gif' : 'image'),
          video: i.video || null,
          bg: i.poster,       // body 底 / 缩略图统一用海报；视频另走 lab.setVideo
          poster: i.poster,
          preview: i.preview,
          name: i.title,
          ...
          type: i.type,
          note: i.note,
          tokens: null,       // 懒加载：applySkin 时经 /we-wallpaper/color 取回
        }));
      if (we.length) skins = skins.concat(we);
      if (!skins.length) return;
      const saved = skins.find(s => s.id === activeId);
      applySkin(saved || skins[0]);
    };
    loadSkins().catch(err => console.warn('[dsh-capsule-skin] 清单加载失败', err));
