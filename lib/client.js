...
body[data-ds-skin="capsule"][data-ds-adaptive-text="light"] {
  --dsw-alias-label-primary: rgb(20, 20, 24);
  --dsw-alias-label-secondary: rgba(20, 20, 24, 0.72);
}
body[data-ds-skin="capsule"][data-ds-adaptive-text="dark"] {
  --dsw-alias-label-primary: rgb(245, 245, 248);
  --dsw-alias-label-secondary: rgba(245, 245, 248, 0.72);
}
`;
...
function saveSettings(s) {
  try { localStorage.setItem(LAB_STORAGE_KEY, JSON.stringify(s)); } catch (e) {}
}
...
// 阈值取 0.5（可调）：背景亮度 > 0.5 用深色文字，<= 0.5 用浅色文字。
const TEXT_LUM_THRESHOLD = 0.5;
...
function createWallpaperLab(ctx, getBgColor) {
  ...
  function scheduleSave() {
    ...
    saveTimer = setTimeout(() => { saveTimer = null; saveSettings(settings); }, 120);
  }
  ...
  function dispose() {
    ...
    if (saveTimer) { clearTimeout(saveTimer); saveTimer = null; saveSettings(settings); } // 冲刷未落盘的防抖保存
...
function buildPicker(container, skins, activeId, onPick, lab) {
  ...
  labEntry.style.borderBottom = '1px solid rgba(255,236,235,0.1)';
  ...
  labEntry.addEventListener('click', () => {
    ...
    back.style.borderBottom = '1px solid rgba(255,236,235,0.1)';
