...
// 浮层定位：底部锚定"壁纸皮肤"按钮上方 8px（向上展开，永不遮挡按钮）；
// 高度上限 = min(视口 1/3, 按钮以上可用空间)，按钮位置异常偏高时防溢出视口顶部。
// 内联样式优先于 SKIN_CSS 里的兜底值（bottom:64px / max-height:33vh）。
function positionPicker(picker) {
  const btn = document.querySelector('[data-capsule-skin-btn]');
  if (!btn || !picker) return;
  const rect = btn.getBoundingClientRect();
  picker.style.bottom = (window.innerHeight - rect.top + 8) + 'px';
  picker.style.maxHeight = Math.min(window.innerHeight * 0.33, Math.max(120, rect.top - 16)) + 'px';
}
