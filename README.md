...
- 左栏为**大胶囊**（去掉了与中间栏的分界线，四边留出壁纸呼吸区），顶栏（会话标题栏）同为胶囊悬浮条
- **胶囊模糊半径可调**：设置 → 壁纸皮肤 → 外观，一个滑块全局联动（0–40px，默认 16px）
- 项目胶囊物理包含会话胶囊（大胶囊包小胶囊）
- 壁纸作为界面背景，主题色由壁纸自动提取（Google Material You 同款取色管线）
- 热切换：**设置 → 壁纸皮肤**，**壁纸 × UI 合成预览图**（壁纸为底、叠加迷你 UI 骨架，模糊随全局设置联动）点选即换（原左下角「🖼 壁纸皮肤」按钮已并入设置）
...
1. 重启 dsh web 后，打开左下角 **「⚙️ 设置」** → 左侧导航 **「壁纸皮肤」**
2. 在 **「📁 壁纸目录」** 输入框填你的壁纸文件夹（默认已按平台填好），点 **「保存目录」**
3. 点 **「🔄 扫描新壁纸」**，等待处理完成
4. 重新打开列表，新壁纸出现（带缩略图），**点选即切换**，选择自动记忆（localStorage）
5. 「外观 → 胶囊模糊」滑块可全局调节左栏 / 顶栏 / 输入托盘等毛玻璃强度，实时生效
...
| 现象 | 处理 |
|---|---|
| 装完设置里没有「壁纸皮肤」页 | 重启 dsh web + 浏览器强刷（组合层变更不热更新） |
...
│   └── client.js       # 浏览器半：皮肤应用（overrideTokens）+ 设置页「壁纸皮肤」视图（列表/美化）+ 模糊半径 + 目录配置 + 扫描按钮
├── assets/
...
- **取景**：整张壁纸预览（长/宽壁纸视口裁剪外的区域也能取到），拖拽移动、滚轮缩放选择显示区域，可一键重置
- **一键预设**：原图 / 柔光 / 胶片 / 黑白 / 冷调 / 暖阳 / 暗夜
- 设置自动保存到 `localStorage`；自动跟随壁纸切换（与胶囊皮肤联动）

入口：设置 → 壁纸皮肤 → 「🎨 壁纸美化」进入美化视图（与皮肤列表共用同一设置页容器，列表 ↔ 美化两视图切换）。
